...
");
        ...
        {
            _sbBusinessClass.Append($@"
        public static bool Is{TableSingularName}Exist({_PrimaryKeyColumn.ColumnDataType} {_PrimaryKeyColumn.ColumnName})
        {{
        ...
        private void _GenerateMethod_FindbyColumn(clsColumn column)
        {
            _sbBusinessClass.AppendLine($"\r\n        public static {TableClassName} FindBy{column.ColumnName}({column.ColumnDataType} {column.ColumnName})");
            _sbBusinessClass.AppendLine("        {");
            ...
            _sbBusinessClass.AppendLine($"\r\n            bool IsFound = {_DALClassName}.Get{TableSingularName}By{column.ColumnName}({_GetParameterList2(column,true, true, false)});");
            _sbBusinessClass.AppendLine($"\r\n            if(IsFound)");
            _sbBusinessClass.AppendLine($"                return new {TableClassName}({_GetParameterList2(column,true, false, false)});");
            _sbBusinessClass.AppendLine($"            else");
            _sbBusinessClass.AppendLine($"                return null;");
            _sbBusinessClass.Append("        }");
        }
        private void _GenerateFunction_FindByColumn()
        {
            foreach (clsColumn column in _ColumnsList)
            {
                _GenerateMethod_FindbyColumn(column);
            }
        }
